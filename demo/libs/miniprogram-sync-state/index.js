...
function connect(mapStateToData, mapMethodToPage) {
    if (mapStateToData !== undefined && !(0, _utils.isFunction)(mapStateToData)) {
        (0, _utils.err)('connect first param accept a function, but got a ' + (typeof mapStateToData === 'undefined' ? 'undefined' : _typeof(mapStateToData)));
    }
    if (mapMethodToPage !== undefined && !(0, _utils.isFunction)(mapMethodToPage)) {
        (0, _utils.err)('connect second param accept a function, but got a ' + (typeof mapMethodToPage === 'undefined' ? 'undefined' : _typeof(mapMethodToPage)));
    }
    return function (pageObject) {
        if (!(0, _utils.isObject)(pageObject)) {
            (0, _utils.err)('page object connect accept a page object, but got a ' + (typeof pageObject === 'undefined' ? 'undefined' : _typeof(pageObject)));
        }
        // map state to data
        ...
        for (var dataKey in dataMap) {
            if (pageObject.data.hasOwnProperty(dataKey)) {
                (0, _utils.warn)('page object had data ' + dataKey + ', connect map will cover this prop.');
            }
            pageObject.data[dataKey] = dataMap[dataKey];
        }
        var methodMap = mapMethodToPage ? mapMethodToPage(setState, _state) : {};
        // map method to page
        for (var methodKey in methodMap) {
            if (pageObject.hasOwnProperty(methodKey)) {
                (0, _utils.warn)('page object had method ' + methodKey + ', connect map will cover this method.');
            }
        ...
        pageObject.onLoad = function (options) {
            ...
            if (!~_subjects.indexOf(this)) {
                var stateMap = shallowDiffData(this.data, mapStateToData(_state));
                stateMap && this.setData(stateMap);
                _subjects.push(this);
                _observers.push(function () {
                    var stateMap = shallowDiffData(_this.data, mapStateToData(_state));
...
function connectComponent(mapStateToData, mapMethodToPage) {
    ...
    return function (pageObject) {
        ...
        var attached = pageObject.hasOwnProperty('lifetimes') && pageObject.lifetimes.attached || pageObject.attached;
        var detached = pageObject.hasOwnProperty('lifetimes') && pageObject.lifetimes.detached || pageObject.detached;
        ...
        var attachedCache = function attachedCache(options) {
            ...
            if (!~_subjects.indexOf(this)) {
                ...
                _subjects.push(this);
                _observers.push(function () {
                    var stateMap = shallowDiffData(_this2.data, mapStateToData(_state));
         ...
         * 兼容2.2.3以下版本
         */
        if (pageObject.hasOwnProperty('lifetimes') && pageObject.lifetimes.attached) {
            pageObject.lifetimes.attached = attachedCache;
        } else {
            pageObject.attached = attachedCache;
        }
        if (pageObject.hasOwnProperty('lifetimes') && pageObject.lifetimes.detached) {
...
function createStore(state) {
    if (!(0, _utils.isObject)(state)) (0, _utils.err)('init state accept a object\uFF0Cbug get a ' + (typeof state === 'undefined' ? 'undefined' : _typeof(state)));
    if (_state) (0, _utils.warn)('there are multiple store active. This might lead to unexpected results.');
